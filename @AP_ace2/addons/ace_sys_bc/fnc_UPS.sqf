...
if (!isServer && {!("force" in _this)}) exitWith {};
...
if (_initpos!="ORIGINAL") then {
	...
	while {_try<20} do {
		...
		if (_isplane || {_isboat} || {!(surfaceiswater _currPos)}) then {
			if ( _bldpos>0 && {_initpos=="RANDOM" || {_initpos=="RANDOMUP"}}) then {_try=99};
			if ( _bldpos==0 && {_initpos=="RANDOM" || {_initpos=="RANDOMDN"}}) then {_try=99};
		};
		_try=_try+1;
	};
	if (_bldpos==0) then {
...
while {_loop} do {
	...
	if (_issoldier && {!(_exit) && {(count _enemies)>0}}) then {
		...
		if (_waiting<=0) then {
			...
			if (_makenewtarget) then {
				if ((_nomove=="NOMOVE") && {_timeontarget>_alerttime}) then {
					...
					_targetPos=_currPos;
					_tries=0;
					while { _tries<20 && {([_currPos,_targetPos] call KRON_distancePosSqr) < _mindist}} do {
						_tries=_tries+1;
