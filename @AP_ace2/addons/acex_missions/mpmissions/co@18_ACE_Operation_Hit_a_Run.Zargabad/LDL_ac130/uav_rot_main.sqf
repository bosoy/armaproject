...
//local variables
private["_display", "_logic_plane", "_ang", "_dif", "_pos", "_cam", "_screen", "_secTotal"];
...
[]spawn
{
	private["_display", "_screen", "_timeNow"];
	...
	_screen = worldToScreen (getPos LDL_logic_object);
     ...
     (_display displayCtrl 1023) ctrlSetPosition [(_screen select 0)-SafeZoneW/64,(_screen select 1)-SafeZoneH/64];
     (_display displayCtrl 1023) ctrlCommit 0; 
	
	while {!LDL_ac130_abort} do
	{
		waitUntil{(LDL_equippedWeapon == 0 || LDL_ac130_abort)};
		
		_timeNow = time;
		
		while{_timeNow+1 > time && !LDL_ac130_abort && LDL_equippedWeapon == 0} do
		{
			sleep 0.01;
		...
		(_display displayCtrl 1023) ctrlSetPosition [0.5-SafeZoneW/64, 0.5-SafeZoneH/64];
		(_display displayCtrl 1023) ctrlCommit 0.5;
...
while {dialog && alive LDL_ac130_plane && alive player && !LDL_ac130_abort} do
{
	...
	/*
	if(LDL_equippedWeapon != 0) then
	{
		(_display displayCtrl 1023) ctrlSetPosition [-1,-1];
		(_display displayCtrl 1023) ctrlCommit 0; 
		(_display displayCtrl 1024) ctrlSetPosition [-1,-1];
		(_display displayCtrl 1024) ctrlCommit 0; 
		(_display displayCtrl 1025) ctrlSetPosition [-1,-1];
		(_display displayCtrl 1025) ctrlCommit 0;
	};
	*/
	
	LDL_logic_object setPos screenToWorld [0.5,0.5];
	
	if(LDL_equippedWeapon != 1) then
	{
		(_display displayCtrl 1026) ctrlSetPosition [-1,-1];
		(_display displayCtrl 1026) ctrlCommit 0;
	};
	
	//HUD
	#include "UAV\uav_HUD.sqf"
