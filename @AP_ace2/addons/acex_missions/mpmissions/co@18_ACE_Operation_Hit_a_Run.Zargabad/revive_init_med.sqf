...
_JIP_respawns					= [(paramsarray select 3),30];			//array no.63 - 0 - off or the number of lives players receive when they join in progress , JIP time after the mission starts 
_revive_timer 					= 1;	 			//array no.7  - Whether you want to limit the amount of timer a player has while unconscious
_revive_time_limit 				= (paramsArray select 4);				//array no.27 - Amount of time a player remains unconscious before respawning or dying
...
_Base_1 						= "Respawn1";			//array no.13 - spawn position names  
_Base_2 						= "Respawn2";	//array no.14
_Base_3 						= "Respawn3";		//array no.15
_Base_4 						= "Respawn4";			//array no.16 
...
_can_revive 					= "GER_Soldier_Medic_EP1";		//array no.18 
_can_revive_2 					= "US_DELTA_FORCE_MEDIC_EP1";	//array no.19
...
_medic_2						= "US_DELTA_FORCE_MEDIC_EP1"; //array no.77 - Used in conjunction with medpacks and bleeding
...
_medic_medpacks 				= 50;				//array no.83 - No of medpacks for units specified in the _medic variables - they are required for reviving units		
_unit_medpacks 					= (paramsarray select 7);				//array no.84 - No of medpacks for units specified in the _can_revive variables - they are required for reviving units			
...
_follow_cam_distance			= 1000;				//array no.32 - the range that unconscious players can spectate friendly units
...
_top_view_height 	   			= 120;				//array no.55 - allows you to set the top down camera height 
...
_respawn_button_timer 			= (paramsArray select 5);				//array no.25 - Time until respawn button appears (0 = approx. 12 seconds), NB: Set to a high number like 100000 seconds if you do not want
_distance_to_friend 			= 100;				//array no.26 - If the closest friendly unit is further than this distance away trigger respawn dialog 
...
_no_team_kills 					= 3;				//array no.99 - Number of teamkills before punishment
// ==================================================================
// CONFIGURABLE OPTIONS FOR HEAL YOURSELF FUNCTION
_heal_yourself 					= 1;				//array no.8  - whether a unit can heal damage to itself
_no_of_heals 					= (paramsarray select 6);				//array no.29 - Number of heals that each player gets during a mission  
