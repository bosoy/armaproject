...
MON_NearestLandTransport = {
	...
	
	{
		...
		if (!(_x call CBA_fnc_locked)  && canMove _x && _transportSoldier >= count (units _npc) && !(typeof _x in MON_bugged_vehicles)
...
MON_NearestsLandTransports = {
		...
		
	{
		...
		if (!(_x call CBA_fnc_locked) && _emptypositions > 0 && canMove _x && !(typeof _x in MON_bugged_vehicles)
...
MON_NearestsLandCombat = {
		...
		
	{
		...
		if (!(_x call CBA_fnc_locked) && _Gunnercount > 0 && canMove _x && !(typeof _x in MON_bugged_vehicles)
			&& (_drivercount > 0 || side _npc == side _x )) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
	}foreach _OCercanos;
...
MON_NearestsStaticWeapons = {
	...
	_distance = _this select 1;					
			...
			
		{
			...
			if (!(_x call CBA_fnc_locked) && alive _x && _emptypositions > 0 && !(typeof _x in MON_bugged_vehicles) ) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
...
MON_Nearestsboats = {
	...
	_distance = _this select 1;					
			...
			
		{
			...
			if (!(_x call CBA_fnc_locked) && _emptypositions > 0 && canMove _x && (_drivercount > 0 || side _npc == side _x )) then { _vehicles = _vehicles + [[_x,_emptypositions]];};
		}foreach _OCercanos;
