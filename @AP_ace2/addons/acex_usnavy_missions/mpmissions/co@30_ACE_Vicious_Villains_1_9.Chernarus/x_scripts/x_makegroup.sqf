private ["_grptype", "_wp_array", "_target_pos", "_numbervehicles", "_type", "_side", "_grp_in", "_vec_dir", "_grpspeed", "_grp", "_pos", "_unit_array", "_leader", "_grpskill", "_fran", "_one", "_vehicles"];
...
switch (_type) do {
	case "patrol": {
		...
		_grp setVariable ["XGRP_POS", _pos];
		_grp setVariable ["XGRP_UNKN", false];
		_grp setVariable ["XGRP_WPARR", _wp_array];
		_grp setVariable ["XGRP_WPPOS", []];
	};
	case "guard": {
		sleep 0.0123;
