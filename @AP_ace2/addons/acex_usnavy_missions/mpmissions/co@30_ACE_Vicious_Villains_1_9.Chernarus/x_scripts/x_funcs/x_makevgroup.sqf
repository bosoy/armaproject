...
_with_gps = if (count _this > 8) then {_this select 8} else {true};
_the_vehicles = [];
...
for "_n" from 0 to _numbervehicles - 1 do {
	...
	_vec_array = [_pos, _dir, _vehiclename, _grp] call BIS_fnc_spawnVehicle;
	...
	sleep 0.543;
	{
		...
		if (!_with_nv) then {if (_x hasWeapon "NVGoggles") then {_x removeWeapon "NVGoggles"}};
		if (!_with_gps) then {if (_x hasWeapon "ItemGPS") then {_x removeWeapon "ItemGPS"}};
