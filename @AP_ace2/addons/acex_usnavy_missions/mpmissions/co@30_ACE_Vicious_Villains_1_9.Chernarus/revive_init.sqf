...
{
	T_Server = true;
	if (!isDedicated) then {T_Client = true};
	T_INIT = true;
} else {
	T_Client = true;
...
waitUntil {T_INIT};
if (T_Client) then {waitUntil {local player};sleep 2;} else {sleep 3};
// =====================================================================================================================
...
_time_b4_JIP_spawn_dialog 		= 10;			//array no.17 - time before the respawn dialog appears for JIP players
_perpetual_server				= 0;				//array no.62 - NOT IMPLEMENTED
// ==================================================================
// LIST OF PLAYABLE UNITS
NORRN_player_units = ["p1","p2","p3","p4","p5","p6","p7","p8","p9","p10","p11","p12","p13","p14","p15","p16","p17","p18","p19","p20","p21","p22","p23","p24","p25","p26","p27","p28","p29","p30"];
// ==================================================================
// WELCOME SCREEN
...
// ==================================================================
// REVIVE OPTIONS
_max_respawns 					= if (!isNil "param2") then {param2} else {7};			//array no.38 - Number of lives per unit
...
_revive_time_limit 				= if (!isNil "param1") then {param1} else {240};			//array no.27 - Amount of time a player remains unconscious before respawning or dying
...
_distance_to_friend 			= 500;				//array no.26 - If the closest friendly unit is further than this distance away trigger respawn dialog
...
_revives_required 				= 4;				//array no.97 - Number of revives required before recieving a bonus life
// ==================================================================
// Team kill function
...
_no_of_heals 					= 0;				//array no.29 - Number of heals that each player gets during a mission
_lower_bound_heal 				= 0.1;				//array no.30 - The damage level range between which the heal action becomes available
_upper_bound_heal 				= 0.9;				//array no.31
// ==================================================================
...
_call_for_AI_help 				= 0;				//array no. 59 - allows AI units to call for help
...
_water_dialog, _unconscious_music, -1, -1, _AI_aware,_AI_cover,_mobile_spawn,_mobile_base_start,_mobile_man,_altUnc_animation,_top_view_height,
