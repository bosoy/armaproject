...
if (!isServer || local player) then {	
	if (side player == WEST) then {
		...
		if (WF_ACE) then
		{
			...
			_m = _m + ["ACE_M252HE_CSWDM"];
			_m = _m + ["ACE_M252WP_CSWDM"];
			_m = _m + ["ACE_M2_CSWDM"];
			_m = _m + ["ACE_TOW_CSWDM"];
			_m = _m + ["ACE_5Rnd_25x59_HEDP_Barrett"];
			_m = _m + ["ACE_30Rnd_556x45_T_G36"];
			_m = _m + ["ACE_20Rnd_762x51_T_HK417"];
			_m = _m + ["ACE_5Rnd_762x51_T_M24"];
			_m = _m + ["ACE_20Rnd_762x51_SB_SCAR"];
		};
		...
		if(WF_ACE) then
		{
			_w = _w + ['ACE_Rucksack_MOLLE_Brown'];
			_w = _w + ['ACE_Coyote_Pack'];
			_w = _w + ['ACE_CharliePack'];
			_w = _w + ['ACE_M252Proxy'];
			_w = _w + ['ACE_M252TripodProxy'];
			_w = _w + ['ACE_M2HBProxy'];
			_w = _w + ['ACE_M3TripodProxy'];
			_w = _w + ['ACE_M220Proxy'];
			_w = _w + ['ACE_M220TripodProxy'];
		}
		else
		{
