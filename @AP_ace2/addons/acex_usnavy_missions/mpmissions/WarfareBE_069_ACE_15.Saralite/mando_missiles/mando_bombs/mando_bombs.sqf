...
group _pilot Move [getPos _target select 0, getPos _target select 1];
group _pilot addWaypoint [getPos _target, 0];
...
while {_continue} do
{
   ...
   while {_continue} do
   {
//      _ang = acos( ((_plane worldToModel getPos _target) select 1)/(_target distance _plane));
      _ang = ((_plane worldToModel getPos _target) select 0) atan2 ((_plane worldToModel getPos _target) select 1);
//      if ((abs(_ang) < 20) && ((_plane distance _target) < 2900)) then
      if ((abs(_ang) < 20) && ((_plane distance _target) < (2900+(getPos _plane select 2)))) then
      {
         _continue = false;
      };
   ...
   {   
      _posp = getPosASL _plane;
      _post = getPosASL _target;
      _disth = sqrt(((getPos _plane select 0) - (getPos _target select 0))^2 + ((getPos _plane select 1) - (getPos _target select 1))^2);
