// by Xeno
private ["_startposi", "_radius", "_number_basic", "_number_specops", "_number_tank", "_number_bmp", "_number_brdm", "_number_uaz_mg", "_number_uaz_grenade", "_number_shilka", "_side", "_type_list_attack", "_trg_center", "_sides", "_random_val", "_trig_array", "_i", "_trigger", "_wp_array", "_xx", "_typeidx", "_number_"];
...
if (typeName _side == "ARRAY") then {
	...
	for "_i" from 0 to (count _sides - 1) do {
		...
		_trigger setTriggerActivation [(_sides select _i), "PRESENT", false];
		call compile format ["_trigger setTriggerStatements[""this"", ""x_finder_%1 = true"", """"];", _random_val];
		[_trig_array , _trigger] call BIS_fnc_arrayPush;
	};
...
_wp_array = if (typeName _radius == "ARRAY") then {
	[_startposi, _radius select 0, _radius select 1, _radius select 2]
} else {
	[_startposi, _radius]
};
...
for "_xx" from 0 to (count _type_list_attack - 1) do {
	...
	call compile format["if (_number_%1 > 0) then {for ""_i"" from 1 to _number_%1 do {[_typeidx select 0, _wp_array, _startposi, _typeidx select 1, ""guard"",_side,objNull,-1.111] execVM ""x_scripts\x_makegroup.sqf"";sleep 1.123;};};",_typeidx select 0];
};
