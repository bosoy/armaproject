...
if((count _temp2 > 0) && (_proVeh > 0)) then
{
	while {_i < count _temp1} do
	{
		...
		if((format["%1",typeof _entry1] in _temp2) && (((leader _group) distance _entry1) < _emptVeh)) then
		{
			if(!(_entry1 call CBA_fnc_locked) && (!(format["%1",typeof _entry1] in DAC_GunAllowed))) then
			{
