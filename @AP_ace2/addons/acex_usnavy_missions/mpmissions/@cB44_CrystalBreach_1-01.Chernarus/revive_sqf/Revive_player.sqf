...
if (local _name) then
{
	...
	if ((_chance_ofDeath select 0) == 1 || (_chance_ofDeath select 1) == 1) then
	{
		...
		_name addEventHandler ["dammaged", {_this exec "revive_sqf\COD\body_damage.sqf"}]; //[unit, selectionName, damage]
...
// Unit alive
	while {alive _name && !_respawn && !_conscious} do
	{
		...
		if (_ACE_mod == 1) then
		{
			...
			if (_hasruck) then
			{
				_ruckType = _name call ACE_Sys_Ruck_fnc_FindRuck;
...
// Unit falls unconscious
	if (_respawn && !_conscious && !_dead) then
	{
		...
		if (_ACE_mod == 1) then
		{
			_nearRucks = nearestObjects [_name, ["ACE_Rucksack_crate"], 5];
			if (count _nearRucks > 0) then
			{
				_nearRuck = _nearRucks select 0;
			} else {
			_nearRuck = objNull;
			};
		};
				...
				count _caseVAC > 0) then
			{
				...
				if (_e >= 1 || _caseVAC_check) then
				{
					...
					if (_ACE_mod == 1) then
					{
						if (_hasruck) then
						{
							_name addWeapon _ruckType;
							_name setVariable ["ACE_RuckMagContents", _ruckMags, true];
							_name setVariable ["ACE_RuckWepContents", _ruckWeps, true];
						};
						if (!isNull _nearRuck) then
						{
							deleteVehicle _nearRuck;
						};
					};
