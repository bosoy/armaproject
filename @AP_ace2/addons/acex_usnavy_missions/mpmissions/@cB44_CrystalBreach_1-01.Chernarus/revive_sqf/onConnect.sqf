...
if (isServer && !local player) then
{
	...
	{
		{if (!isNull (call compile format ["%1", _x])) then {[_x, NORRN_player_units] execVM "revive_sqf\Revive_player.sqf"}}forEach NORRN_player_units;
	};
};

//Initialise revive script for all units in players group if AI is enabled
{if (_AI_disabled == 0 && !isNull (call compile format ["%1", _x]) && (call compile format ["%1", _x]) in (units (group player))) then {[_x, NORRN_player_units] execVM "revive_sqf\Revive_player.sqf"}}forEach NORRN_player_units;
