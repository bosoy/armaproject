...
#define TSD9_ColorScheme_3DControlBackground 40/256, 51/256, 34/256 // A2 Dark green bg
#define TSD9_ColorScheme_3DControlBackgroundAlt (40/256)-TSD9_AltBGDiff, (51/256)-TSD9_AltBGDiff, (34/256)-TSD9_AltBGDiff // pale brown grey
...
TSD9_SetText =
{
  ...
  */
  ...
  //(_idc2 call TSD9_getControl) ctrlSetTooltip _txt2; // doesn't do anything for text ctrl types, only buttons
  ...
  if (not ([_unit] call TSD9_IsVehicle)) then
  {
    ...
    then
    {
      if (_unit == driver _vehicle) then
      {
        ...
        then { _seat = [["", "Pilot", "\CA\ui\data\i_driver_ca.paa"]] }
        else { _seat = [["", "Driver", "\CA\ui\data\i_driver_ca.paa"]] };
      };
...
TSD9_GetGroupDesc =
{
  ...
  if ((TSD9_Page == "Vehicle") && ((_group == grpNull /* doesn't work? */) || (_GroupName == "<NULL-group>"))) then
...
TSD9_JoinGroupByName =
{
  ...
  _groupList = allGroups; // [_UnitList] call TSD9_GetAllGroupsFromUnits;

  {
    _group = _x;
    if (format["%1", _group] == format["%1", _groupName]) exitWith
    {
...
TSD9_InviteAIOrPlayerIntoGroupByName =
{
  ...
  _UnitList = allUnits; //_this select 1;
...
TSD9_AddPlayerStatsRowButtonAction =
{
  ...
  else
  {
    ...
    if ((TSD9_Page == "Team") && _leaderAndYourGroup) then
    {
      ...
      if (isPlayer _player) then
      {
        _command = localize "STR_TSD9_56" /* "Kick" */
      };
    ...
    if ((TSD9_Page == "Team") && _leaderAndOtherGroup) then
    {
      ...
      // Warning: TSD9_AllowPlayerInvites takes precedence over TSD9_AllowPlayerRecruitment
  ...
  if (TSD9_Page == "Vehicle") then
  {
    ...
    if ([_vehicle] call TSD9_IsVehicle) then
    {
      ...
      if ((_dr > 0) || (_gu > 0) || (_co > 0) || (_ca > 0)) then
      {
        ...
        if (_vehicle isKindOf "Air") then { _drType = "Pilot" } else { _drType = "Driver" };
        {
          ...
          if ( (_seatCount > 0)) then
          {
            ...
            for "_i" from 0 to (_seatCount - 1) do
            {
              ...
              [_vehicle, _row, _id, [format["%1 %2", localize "STR_TSD9_59", _seatName], _picture]
              ] call TSD9_AddPlayerStatsRow;
              _row = _row + 1;
              _id = _id + 1;
            };
          };
        ...
        {
          [_vehicle, _row, _id, [
            format["%1 %2 (%3)", localize "STR_TSD9_59", localize "STR_TSD9_60", 1+_i],
...
TSD9_FillClosedGroupsWithAllGroups =
{
  ...
  _AllGroups = allGroups; //[_AllUnits] call TSD9_GetAllGroupsFromUnits;
...
TSD9_CollapseAll =
{
  ...
  _Trig setTriggerStatements [
    "this",
    "[thislist] call TSD9_FillClosedGroupsWithAllGroups; ",
    "" ];
  */
...
TSD9_LocalizeText =
{
  ...
  //(TSD9_IDC_CloseButton call TSD9_getControl) ctrlCommit 0; // TSD9_RscShortcutButton localized text not appearing?
...
//[__FILE__, _this] call fn_DebugAppend; // debug
_this call TSD9_ProcessParameters;

[] call TSD9_DrawPage;

//[__FILE__, allGroups] call fn_DebugAppend;
