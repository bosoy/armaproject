	...
	case "TownAddComplete": {
		...
		for '_i' from 0 to count(_towns)-1 do {
			...
			if (_sideID != sideID) then {
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown",_town getVariable "name"], Format [localize "STR_WF_TaskTown_Display",_town getVariable "name"], Format [localize "STR_WF_TaskTown_Display",_town getVariable "name"]];
		} else {
				_task setSimpleTaskDescription [Format[localize "STR_WF_TaskTown_Complete",_town getVariable "name"], Format [localize "STR_WF_TaskTown_Display",_town getVariable "name"], Format [localize "STR_WF_TaskTown_Display",_town getVariable "name"]];
