...
//    onroad      = Unit will get target destination only on the roads !R
//    nosmoke     = Units will not use smoke when s/o wounded or die. !R
		...
		if (_waiting<0) then {
			...
			{			
				...
				if (format ["%1",_fixedtargetPos] !="[0,0]") then {	
					...
					if (KRON_UPS_Debug>0) then {player sidechat format["nomove: %1",_nomove]};		//!R
