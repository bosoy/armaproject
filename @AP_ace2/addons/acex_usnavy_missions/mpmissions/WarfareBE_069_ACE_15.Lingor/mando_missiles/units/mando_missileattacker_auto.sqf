...
if (typeName (_this select 8) == "ARRAY") then
{
   ...
   if (count (_this select 8) > 2) then
   {
      _min_angv = (_this select 8) select 2;
	  _max_angv = (_this select 8) select 3;
   };  
}
else
{
...
[_disp, _min_angv, _max_angv] spawn
{
   _disp = _this select 0;
   _min_angv = _this select 1;
   _max_angv = _this select 2;			   

   while {alive _disp} do
   {
     _blanco = _disp getVariable "mando_target_auto";
	 if (alive _blanco) then
	 {
                  _posl = _disp worldToModel (getPos _blanco);
                  _ang1 = (_posl select 0) atan2 (_posl select 1);
                  _ang2 = (asin ((_posl select 2)/(_blanco distance _disp))) max _min_angv;
			      _ang2 = _ang2 min  _max_angv;				  
                  _disp animate ["turret", -rad(_ang1)];
                  _disp animate ["gun", rad(_ang2)];				  
                  Sleep 0.025;
      }
	  else
	  {
...
// Sleep between radar lock message and missile
         ...
         _ang2 = (asin ((_posl select 2)/(_blanco distance _disp))) max _min_angv;
		 _ang2 = _ang2 min  _max_angv;
         _ang = getDir _disp + _ang1;
         _angv = _ang2; 
		 while {abs((_disp animationPhase "turret") - (-rad(_ang1))) > 0.1} do
		 {
            _posl = _disp worldToModel (getPos _blanco);
            _ang1 = (_posl select 0) atan2 (_posl select 1);
            _ang2 = (asin ((_posl select 2)/(_blanco distance _disp))) max _min_angv;
            ...
            _selection = "mma_spawn" + str(_quantity-1);		

            _posl = _disp selectionPosition _selection;
	        if (count(_posl - [0]) == 0) then
            {
               _posl = _disp selectionPosition "mma_spawn";
	        };
