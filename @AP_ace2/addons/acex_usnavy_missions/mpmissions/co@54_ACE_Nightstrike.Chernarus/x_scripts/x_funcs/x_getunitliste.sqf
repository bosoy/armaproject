...
switch (_grptype) do {
	case "basic": {_unitliste = [call compile format["d_sleader_%1",_side_char]];_how_many = ceil random 8; if (_how_many < 3) then {_how_many = 3;};_list = call compile format ["d_allmen_%1",_side_char];for "_i" from 0 to _how_many - 1 do {_unitliste set [count _unitliste,(_list select (floor random count _list))];};};
	case "specops": {_how_many = ceil random 5; if (_how_many < 3) then {_how_many = 3;};_list = call compile format ["d_specops_%1",_side_char];for "_i" from 0 to _how_many - 1 do {_unitliste set [count _unitliste,(_list select (floor random count _list))];};};
	case "heli": {_unitliste = [call compile format["d_sleader_%1",_side_char]];_how_many = ceil random 8; if (_how_many < 3) then {_how_many = 3;};_list = call compile format ["d_allmen_%1",_side_char];for "_i" from 0 to _how_many - 1 do {_unitliste set [count _unitliste, (_list select (floor random count _list))];};};
	...
	case "crew": {for "_i" from 1 to 12 do {_unitliste set [count _unitliste, call compile format["d_crewman_%1",_side_char]]};};
};
