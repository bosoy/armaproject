...
while{dialog} do {
	...
	call {
		...
		if(time - _lastAutoUpdateLB > 20) then {
			_lastAutoUpdateLB = time;
			KEGsNeedUpdateLB = true;
			_debugPlayer globalchat "Refeshing Player List";
		};
		...
		_debugplayer globalchat format ["KEGs_cameraIdx = %1 VM_CurrentCameraView = %2 ", KEGs_cameraIdx, VM_CurrentCameraView];
...
//		};
		...
		if(KEGsDroppedCamera) then {
			...
			_cxpos = (vehicle KEGs_target modelToWorld [0,0,0]) select 0;
			_cypos = (vehicle KEGs_target modelToWorld [0,0,0]) select 1;
			_czpos = (vehicle KEGs_target modelToWorld [0,0,0]) select 2;
		};
