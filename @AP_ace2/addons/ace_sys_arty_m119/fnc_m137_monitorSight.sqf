...
waitUntil {
	...
	// player sideChat format["opticPhase: %1", (floor(_gun animationPhase "M137OpticRevolve"))];
	_opticPhase = (floor(_gun animationPhase "M137OpticRevolve")*-1);
	...
	_opticPhaseRaw = (floor(_gun animationPhase "M137OpticRevolve"));
	...
	_traversePhase = _traversePhase + 100;
	_percent = (abs (round _traversePhase))/200;
	_traverseIndicator = "-----^-----";
	_tArray = (toArray _traverseIndicator);
	_tLength = (count _tArray);
	_indicPos = ((round(_percent*_tLength))-1 max 0) min 10;
