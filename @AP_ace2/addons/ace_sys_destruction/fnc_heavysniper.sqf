...
drop [["\ca\Data\ParticleEffects\Universal\Universal",16,12,8], "",
	"Billboard", 1, 1 + random 0.5, [0, 0.7, 0], [0, 2.2 + random 1.3, 0], 0, 0.104, 0.08, 0.2,
...
while {_i < _no} do {
	drop [["\ca\Data\ParticleEffects\Universal\Universal",16,12,8], "",
		"Billboard", 1, 1.5 + random 1, [0, 0.7, 0], [0.7 + random 1.7, 0, 0], 0, 0.104, 0.08, 0.2,
...
if ((abs(_p select 2) < 1) && {!(surfaceiswater _p)}) then {
	...
	while {_i < _no} do {
		drop [["\ca\Data\ParticleEffects\Universal\Universal.p3d", 16, 12, 13, 0], "", "Billboard", 1, 1.5 + random 0.5,
			[(_p select 0)+0.7*(_vec select 0), (_p select 1)+0.7*(_vec select 1), 0], [((_vec select 0) -2 +(random 4))*1.4, ((_vec select 1) -2 +(random 4))*1.4, 0], 0,
