...
while { !isNull _mine } do {
	if (_cl - (player distance _mine) >= 0) then {
		if (typeName (_mine getVariable ["SOAID",false]) == "BOOL" && {typeName (_mine getVariable ["TOAID",false]) == "BOOL"})  then {
			...
			missionNamespace setVariable [format ["SOA_M18_LOCK_%1", _id], false]; // TODO: Dynamically build VARname
			_SOA_ID = player addAction [_A_S, (PATH + "safety_off.sqf"), [_mine, _id], 40, false, false, "","_target == player && {!(_target getVariable ['ace_sys_wounds_uncon',false])}"]; // TODO: Make into function
