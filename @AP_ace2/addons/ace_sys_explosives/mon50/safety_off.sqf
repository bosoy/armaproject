...
if (!(missionNamespace getVariable (format ["SOA_MON50_LOCK_%1", _id]))) then {
	while { !isNull _mine && {time < _clackerResetTime} } do {
		if (_cl - (player distance _mine) >= 0) then {
			if (typeName (_mine getVariable ["TOAID",false]) == "BOOL") then {
				...
				missionNamespace setVariable [format ["SOA_MON50_LOCK_%1", _id], true];
				_TOA_ID = player addAction [_A_S, (__DIR + "\fnc_detonate_MON50.sqf"), _mine, 80, false, false, "","_target == player && {!(_target getVariable ['ace_sys_wounds_uncon',false])}"]; // TODO: Generate detonation function dynamically
	...
	missionNamespace setVariable [format ["SOA_MON50_LOCK_%1", _id], false];
