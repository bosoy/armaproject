...
private ["_irstrobes_inventory", "_has_strobes"];
_irstrobes_inventory = (("IR_Strobe_Target" in (magazines player)) || ("IR_Strobe_Marker" in (magazines player)));
_has_strobes = player getVariable QGVAR(active_strobes);
if (isNil "_has_strobes") then { player setVariable [QGVAR(active_strobes), [], false]; };
	...
	[
		...
		[
			[localize "STR_DSS_ACE_IRSTROBE" + " >", "", "", "",
				...
				-1, 1, _irstrobes_inventory && ACE_SELFINTERACTION_RESTRICTED]
		]
	]
];
