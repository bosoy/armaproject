...
if(!isDedicated) then {
    ...
    _o = 1;
    waitUntil {
        if (visibleMap && {QUOTE(ITEMNAME) in (weapons player)} && {!ace_sys_spectator_SPECTATINGON}) then {
			...
			if (ace_sys_spectator_SPECTATINGON || {!(QUOTE(ITEMNAME) in (weapons player))}) then {
				deleteMarkerLocal QUOTE(DOUBLES(ADDON,compassRuler));
                deleteMarkerLocal QUOTE(DOUBLES(ADDON,compassRose));
			};
            _actionId = -1;
