...
{
	//[_MTrigger]execVM "\DAC_Source\Marker\DAC_InitMarkerG.sqf";
}
else
{
	...
	{
		_u = _MCM select _i;
		if(_u == 0) then {_MUSide = _MUSide + [(_MSides select _i)]};
		_i = _i + 1;
	};
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									{if(!(_x in _s)) then {_s = _s + [_x];[_x,1] spawn DAC_fUnitMarker}}foreach _r;
								};
							};
							_i = _i + 1;
							sleep 0.01;
