	...
	while {_run > 0} do
	{
		switch (_run) do
		{
			...
			case 2:	{
						...
						if((count _TempUnitsB > 0) && (!(((_TempUnitsB select 0) select 0) == "NULL"))) then
						{
							...
							else
							{
								...
								{
									_campScr = (_TempUnitsB select 7);DAC_Init_Group = DAC_Init_Group + [_randomcount];
									...
									while {_we < count _wset} do
									{
										...
										_ab setdir ((_wset select _we) select 3);_ammoarray = _ammoarray + [_ab];
									...
									if(DAC_Marker > 0) then
									{
										if((DAC_Marker_Val select 13) > 0) then
										{
											...
											_mf setMarkerSizelocal [0.8,0.8];_mf setMarkerColorlocal (_DAC_MColor select _groupside);_isM = _isM + [_mf];
										};
									};
			...
			case 4:	{
						...
						else
						{
							...
							while {_we < count _wset} do
							{
								//sleep 0.01;
								_ab = ((_wset select _we) select 0) createvehicle [(_wpstart select 0) + ((_wset select _we) select 1),(_wpstart select 1) + ((_wset select _we) select 2),0];
								_ab setdir ((_wset select _we) select 3);_objarray = _objarray + [_ab];
								_tempST = _tempST + [_ab,((_wset select _we) select 4)];_staarray = _staarray + [_ab];
								_ab setpos [(_wpstart select 0) + ((_wset select _we) select 1),(_wpstart select 1) + ((_wset select _we) select 2),0];
								_we = _we + 1;
							};
			...
			case 5:	{
						...
						//#createVEG
						//-----------------------------
						{if((_x iskindof "Air") || (_x iskindof "Car") || (_x iskindof "Tank")) then {DAC_VehicleInUse = DAC_VehicleInUse + [_x]}} foreach _objarray;
			...
			case 7:	{
						...
						else
						{
							...
							while {_we < count _wset} do
							{
								...
								_eman allowFleeing _setFleeing;_eman setSkill _setskill;_eman setRank _setrank;
								_eman setTargetAge "10 MIN";
								_solarray = _solarray + [_eman];_we = _we + 1;
							};
			...
			case 8:	{
						...
						switch (_groupside) do
						{
							case 0:	{
										...
										sleep 0.01;
										{DAC_East_Units = DAC_East_Units + [_x]} foreach units _group;_wside = 0;
										if(_cs2 == 0) then {DAC_Spawn_CampsE = DAC_Spawn_CampsE + [[_homezone,_camp,_build,_spawnpro,_spawntyp,[_spawncount,0],_vA]]};
									};
							case 1:	{
										...
										sleep 0.01;
										{DAC_West_Units = DAC_West_Units + [_x]} foreach units _group;_wside = 1;
										if(_cs2 == 0) then {DAC_Spawn_CampsW = DAC_Spawn_CampsW + [[_homezone,_camp,_build,_spawnpro,_spawntyp,[_spawncount,0],_vA]]};
									};
							case 2:	{
			...
			case 9	:{
						...
						{
							if(!(format["%1",_groupvar] == "")) then {call compile format["%1 = %1 + [_group]",_groupvar]};
						}
						else
						{
							[_wside,_camp]execVM "\DAC_Source\Marker\DAC_CampMarker.sqf";
						};
			...
			case 11:{
						...
						else
						{
							...
							]	execVM "\DAC_Source\Scripts\DAC_No_Camp.sqf";
							_run = 0;
						};
					};
			Default{};
