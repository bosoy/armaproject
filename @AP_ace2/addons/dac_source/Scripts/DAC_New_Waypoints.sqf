	...
	switch (_run) do
	{
		case 1:	{
					if(!(_DAC_WP_Typ == 1)) then
					{
						...
						_DAC_EditZone = _zone;
						if(((triggerArea _zone) select 0) == (_size select 0)) then
						{
						...
						{
							_DAC_ZoneRadiusX = _size select 1;
							_DAC_ZoneRadiusY = _size select 0;
						};
						_run = 3;
					}
					else
					...
					else
					{
						...
						else
						{
							while{(((getwppos[_log,_wn + 1]) select 0)+((getwppos[_log,_wn + 1]) select 1) != 0)} do
							{
								...
								_TempArray = _TempArray + [[_checkwp select 0,_checkwp select 1]];
