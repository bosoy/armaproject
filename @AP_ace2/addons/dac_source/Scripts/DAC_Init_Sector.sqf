...
if(local player) then
{
	...
	{DAC_Sector_Values = DAC_Sector_Values + [_x]} foreach [_eS,_fS];
	if(DAC_Sector_Values select 6) then {execVM "\DAC_Source\Scripts\DAC_Hint_Sectors.sqf"};
};
sleep 1;
if(isServer) then
{
	...
	while {_i < count _zones} do
	{
		...
		if(_s != "") then {[_z,_s] execVM "\DAC_Source\Scripts\DAC_Sector_Action.sqf"};
		_i = _i + 1;
	};
};