...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 3:	{
					...
					if(_DAC_WP_Typ == 1) then {{[_x]execVM "\DAC_Source\Scripts\DAC_Init_Waypoint.sqf"} foreach _ScriptArray};
					if(!(_DAC_WP_Typ in DAC_Init_WP)) then {DAC_Init_WP = DAC_Init_WP + [_DAC_WP_Typ]};
						...
						else
						{
							...
							switch (_DAC_WP_Typ) do
							{
								...
								case 4:	{
											...
											else
											{
												...
												DAC_WP_Pool_H = DAC_WP_Pool_H + [[_DAC_EnemyZone,_DAC_ZoneMode,0,0,0]];
											};
										};
								case 5:	{
											...
											DAC_WP_Pool_B = DAC_WP_Pool_B + [[_DAC_EnemyZone,_DAC_ZoneMode,0,0,0]];
										};
								Default {};
							};
							if(_run == 20) then
							...
							else
							{
								...
								else
								{
									...
									{
										call compile format["DAC_WP_Pool_%1 = DAC_WP_Pool_%1 + [[_DAC_EnemyZone,_DAC_ZoneMode,_DAC_GroupSol,_DAC_GroupVeh,_DAC_GroupTan,_DAC_GroupAir,_DAC_ZoneSide,_CheckRadius2]]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
										_run = 5;_go = 1;
									};
								};
							};
						...
						switch (_go) do
						{
							...
							case 3:	{
										while{(((getwppos[_log,_wn + 1]) select 0)+((getwppos[_log,_wn + 1]) select 1) != 0)} do
										{
											...
											call compile format["DAC_WP_Pool_%1 = DAC_WP_Pool_%1 + [[_checkwp select 0,_checkwp select 1]]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
											...
											if(DAC_Marker > 0) then
											{
												if((DAC_Marker_Val select 1) > 0) then
												{
													...
													if((DAC_Marker_Val select 1) == 1) then {_MTemp = _MTemp + [_ma]};
												};
											};
										};
										_go = 4;
		...
		case 10:{
					...
					else
					{
						...
						if(_run != 7) then
						{
							...
							if(_DAC_Count_WP == 0) then
							{
								call compile format["DAC_WP_Pool_%1 = DAC_WP_Pool_%1 + [[0,0]]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
							}
							else
							{
								if(_wpnr < _DAC_Count_WP) then {call compile format["DAC_WP_Pool_%1 = DAC_WP_Pool_%1 + [[_RandomX,_RandomY]]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)]};
							};
							if(DAC_Marker > 0) then
							{
							...
							_TempArray = _TempArray + [[_RandomX,_RandomY]];
		...
		case 12:{
					...
					else
					{
						...
						if(_DAC_WP_Typ == 4) then {DAC_WP_Pool_H = DAC_WP_Pool_H + [[_DAC_EnemyZone,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,_countairwp]]};
						if(_DAC_WP_Typ == 5) then {DAC_WP_Pool_B = DAC_WP_Pool_B + [[getpos _DAC_EnemyZone,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,0]]};
						_run = 4;
					};
				};
		case 13:{
					...
					{deletevehicle _x} foreach [_DumyA,_DumyB,_Null];
					if(count _MTemp > 0) then {[_MTemp]execVM "\DAC_Source\Marker\DAC_DeleteMarker.sqf"};
					if(!(_DAC_WP_Typ == 1)) then
					{
						_run = 0;
					...
					else
					{
						...
						{
							[1]execVM "\DAC_Source\Scripts\DAC_Create_Groups.sqf";
							execVM "\DAC_Source\Marker\DAC_LinkMarker.sqf";
