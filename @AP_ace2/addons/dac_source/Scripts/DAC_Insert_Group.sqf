...
else
{
	...
	if(count _wptemp > 0) then
	{
		...
		while {_run == 0} do
		{
			...
			else
			{
				...
				while {_go} do
				{
					...
					if(_id == 0) then {_wparray = _wparray + [format["%1",_newwp]];_go = false} else {_id = 0};
				};
			};
		};
		sleep 0.1;
		_vehA = [];_vehvalue = [];_run = 0;
		{
			...
			{
				_vehA = _vehA + [vehicle _x];
				DAC_VehicleInUse = DAC_VehicleInUse + [vehicle _x];
		...
		else
		{
			...
			while {_run == 0} do
			{
				if(!(_group in DAC_All_Groups)) then {DAC_All_Groups = DAC_All_Groups + [_group]};
				if(_side == 0) then {{DAC_East_Units = DAC_East_Units + [_x]} foreach units _group};
				if(_side == 1) then {{DAC_West_Units = DAC_West_Units + [_x]} foreach units _group};
				if(_side == 2) then
				{
				...
				if((_options select 2) == 0) then {DAC_No_Support = DAC_No_Support + [_group]};
