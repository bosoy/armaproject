...
switch (_side) do
{
	case 0:	{
				...
				_addUnit = format["DAC_East_Units = DAC_East_Units + [_unit]"];
				_addPlayer = format["if(!((call compile _x) in DAC_West_Units)) then {DAC_West_Units = DAC_West_Units + [(call compile _x)]}"];
				_addLeader = format["DAC_East_Units = DAC_East_Units + [_leader]"];
	...
	case 1:	{
				...
				_addUnit = format["DAC_West_Units = DAC_West_Units + [_unit]"];
				_addPlayer = format["if(!((call compile _x) in DAC_East_Units)) then {DAC_East_Units = DAC_East_Units + [(call compile _x)]}"];
				_addLeader = format["DAC_West_Units = DAC_West_Units + [_leader]"];
...
{if((!(typeof vehicle _x == typeof _x)) && (!(vehicle _x in _vehA))) then {_vehA = _vehA + [vehicle _x]}} foreach units _group;
...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 2:	{
					...
					{
						DAC_Min_Groups = DAC_Min_Groups + [_group];_run = 3;
					};
				};
		case 3:	{
					while {_run == 3} do
					...
					while {_gnr < count units _group} do
					{
						...
						if((alive _unit) && (!(_unit == leader _group))) then
						{
							if((_typ == 1) || (_typ == 3)) then
							{
								...
								{
									_utyp = typeof _unit;_skill = skill _unit;_dam = getdammage _unit;_UA = _UA + [_unit];
					...
					else
					{
						...
						while {_gnr < count _grouparray} do
						{
							...
							else
							{
								...
								_gnr = _gnr + 5;_set = _set + [_unit];call compile _addunit;
