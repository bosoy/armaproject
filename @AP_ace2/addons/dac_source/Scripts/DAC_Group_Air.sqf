	...
	while {_run > 0} do
	{
		switch (_run) do
		{
			...
			case 3:	{
						...
						DAC_VehicleInUse = DAC_VehicleInUse + [_vehc];
			...
			case 4:	{
						...
						if((_vehc emptyPositions "cargo") > 1) then {_vehcargo = 1} else {if(!(_group in DAC_All_Groups)) then {DAC_All_Groups = DAC_All_Groups + [_group]}};
						...
						if((_vehcargo == 1) && (_groupside < 3)) then
						{
							...
							]	execVM "\DAC_Source\Scripts\DAC_Group_Soldier.sqf";
						};
