...
if(DAC_Code < 2) then
{
	[] execVM "\DAC_Source\Scripts\DAC_Init_Creator.sqf";
	if((count DAC_VehAllowed) > 0) then {[] execVM "\DAC_Source\Scripts\DAC_Vehicle_check.sqf"};
	DAC_Dyn_Weather execVM "\DAC_Source\Scripts\DAC_Dyn_Weather.sqf";if(DAC_Direct_Start) then {DAC_Basic_Value = 1};
	...
	{
		[] execVM "\DAC_Source\Scripts\DAC_Server_check.sqf";
		if(local player) then {[] spawn DAC_fClientCheck};
		if(DAC_Player_Marker) then {if(local player) then {[player] execVM "\DAC_Source\Marker\DAC_PlayerMarker.sqf"}};
		if((local player) && (DAC_Join_Action)) then {[] execVM "\DAC_Source\AI_1\DAC_Check_Action.sqf"};
		if(DAC_Marker > 0) then
		{
			if(DAC_Marker_Val select 3 > 0) then
			{
				if(local player) then {[player,1,(group player)] execVM "\DAC_Source\Marker\DAC_GroupMarker.sqf"};
			}
			else
			{
				if(local player) then {[]execVM "\DAC_Source\Marker\DAC_InitMarker.sqf"};
			};
		};
	};
}
else
{
	...
	DAC_Dyn_Weather execVM "\DAC_Source\Scripts\DAC_Dyn_Weather.sqf";
	if(DAC_Player_Marker) then {if(local player) then {[player] execVM "\DAC_Source\Marker\DAC_PlayerMarker.sqf"}};
	_sum = time;
	sleep 3;
	if(DAC_Code == 3) then
	{
		...
		if(DAC_Join_Action) then {[] execVM "\DAC_Source\AI_1\DAC_Check_Action.sqf"};
	}
	else
	{
		waituntil {(DAC_Basic_Value > 0)};
