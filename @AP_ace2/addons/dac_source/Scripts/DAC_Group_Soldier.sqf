	...
	while {_run > 0} do
	{
		switch (_run) do
		{
			...
			case 9:	{
						...
						_group selectLeader (units _group select 0);if(!(_group in DAC_All_Groups)) then {DAC_All_Groups = DAC_All_Groups + [_group]};
						sleep 0.01;
						...
						if(DAC_Marker_Val select 3 > 0) then {[_leaderman,1,_group] execVM "\DAC_Source\Marker\DAC_GroupMarker.sqf"};
