	...
	switch (_run) do
	{
					...
					{
						{
							...
							{
								call compile format["DAC_%1_Units = DAC_%1_Units + [_x]",(_sides select _i)];
							};
						}	foreach call compile "DAC_Players";
						sleep 0.5;
		...
		case 3:	{
					while {_i < count _sides} do
					{
						...
						_temparray = [];
						{
							if(!(format["%1",_x] == "<NULL-object>")) then {if((alive _x) && (!(format["%1",group _x] == "<NULL-group>"))) then {_temparray = _temparray + [_x]}};
