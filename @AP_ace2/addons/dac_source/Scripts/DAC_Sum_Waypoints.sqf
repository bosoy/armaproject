...
{
	_ZV = true;while{_h < count DAC_Zones} do {_zData = _zData + [((DAC_Zones select _h) select 0)];_h = _h + 1};
}
else
{
	while{_h < count DAC_Zones} do {_zData = _zData + [((DAC_Zones select _h) select 0)];if(((DAC_Zones select _h) select 0) != (_zones select _h)) then {_g = _g + 1};_h = _h + 1};
