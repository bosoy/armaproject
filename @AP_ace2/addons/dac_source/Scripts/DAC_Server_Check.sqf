...
{
	{
		if((!(format["%1",call compile _x] == scalar)) && (!(format["%1",call compile _x] == "<NULL-object>")) && (!(format["%1",call compile _x] == "scalar"))) then
		{
			...
			{
				DAC_Players = DAC_Players + [call compile _x];
			};
			switch (side (call compile _x)) do
			{
				case "WEST":{if(!((call compile _x) in DAC_West_Units)) then {DAC_West_Units = DAC_West_Units + [(call compile _x)]}};
				case "EAST":{if(!((call compile _x) in DAC_East_Units)) then {DAC_East_Units = DAC_East_Units + [(call compile _x)]}};
				case "GUER":{
								if(DAC_Res_Side == 0) then
								{
									if(!((call compile _x) in DAC_West_Units)) then {DAC_West_Units = DAC_West_Units + [(call compile _x)]}
								}
								else
								{
									if(!((call compile _x) in DAC_East_Units)) then {DAC_East_Units = DAC_East_Units + [(call compile _x)]}
								};
							};
				Default {};
			};
	...
	if(format["%1",DAC_Player_SupGroup] != "") then
	{
		...
		DAC_Hit_Groups = DAC_Hit_Groups + [_val2];
