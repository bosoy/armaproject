...
while {_run > 0} do
{
	switch (_run) do
	{
						...
						else
						{
							...
							if(DAC_Marker > 0) then
							{
								if((DAC_Marker_Val select 13) > 0) then
								{
									...
									_mf setMarkerColorlocal (_DAC_MColor select _groupside);_isM = _isM + [_mf];
								};
							};
		...
		case 3:	{
					...
					if(_cs2 == 0) then
					{
						DAC_Spawn_CampsE = DAC_Spawn_CampsE + [[_homezone,_camp,_camp,_spawnpro,_spawntyp,[_spawncount,0],[]]];
					};
					_run = 7;
				};
		case 4:	{
					...
					if(_cs2 == 0) then
					{
						DAC_Spawn_CampsW = DAC_Spawn_CampsW + [[_homezone,_camp,_camp,_spawnpro,_spawntyp,[_spawncount,0],[]]];
					};
					_run = 7;
				};
		case 5:	{
