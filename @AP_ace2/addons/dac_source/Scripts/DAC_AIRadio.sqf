		...
		if((count (call compile _val3)) > 1) then
		{
			...
			if(count _array < (DAC_Radio_Settings select 2)) then
			{
				...
				if(_action) then
				{
					...
					sleep 0.1;
					_array = _array + [format["%1_%2",_val1,_dist]];
					...
					if(count _array > 0) then
					{
						...
						if((_val7 select 1) == 95) then
						{
							...
							{
								_r = [];_i = 2;while{(_i < count _val7)} do {_r = _r + [(_val7 select _i)];_i = _i + 1};
						...
						else
						{
							...
							if((typeName _dist) != "ARRAY") then
							{
								_r = [];_i = 3;while{(_i < count _val7)} do {_r = _r + [(_val7 select _i)];_i = _i + 1};
								_dist = call compile (toString _r);
							};
						};
