		...
		switch (_run) do
		{
			...
			case 2:	{
						...
						else
						{
							...
							else
							{
								...
								DAC_Init_Group = DAC_Init_Group + [_randomcount];DAC_Init_Values set [0,(DAC_Init_Values select 0) + 1];
								...
								{if(!(_x in DAC_VehAllowed)) then {DAC_VehAllowed = DAC_VehAllowed + [_x]}} foreach _TempUnitsB;
								if(!(DAC_InSpawn)) then
								{
			...
			case 3:	{
						...
						else
						{
							...
							else
							{
								...
								_vehc = _vehicle createvehicle _startpos;[_vehc,_delval0,_delval1,2,_ma,_newgroup,_delval2] spawn DAC_fDeleteUnit;DAC_VehicleInUse = DAC_VehicleInUse + [_vehc];
								...
								if(_WeapList > 0) then {{[_x,format["%1",_WeapList]] execVM "\DAC_Source\Scripts\DAC_Change_Weapons.sqf"}foreach _crewA;_WeapList = 0};
			...
			case 6:	{
						...
						else
						{
							...
							if(!(_group in DAC_All_Groups)) then {DAC_All_Groups = DAC_All_Groups + [_group]};{_x setTargetAge "30 MIN"} foreach units _group;
							if(_groupside == 0) then
							{
								{DAC_East_Units = DAC_East_Units + [_x]} foreach units _group;
							}
							else
							{
								if(_groupside == 1) then
								{
									{DAC_West_Units = DAC_West_Units + [_x]} foreach units _group;
								}
								else
								{
			...
			case 7:	{
						...
						if(count units _group > 0) then
						{
							...
							call compile format["DAC_GroupPool%1 = DAC_GroupPool%1 + [_group]",((_sidearray select 1) select _groupside)];
						};
