...
while {_l < count _TempArray} do
{
	...
	{
		{if(!(_x in DAC_VehAllowed)) then {DAC_VehAllowed = DAC_VehAllowed + [_x]}} foreach _TempList;
	};
	_l = _l + 1;
};
	...
	while {_i < count _UnitList} do
	{
		...
		else
		{
			...
			else
			{
				...
				{
					case "WEST":	{if(!(_entry in DAC_West_Units)) then {DAC_West_Units = DAC_West_Units + [_entry]}};
					case "EAST":	{if(!(_entry in DAC_East_Units)) then {DAC_East_Units = DAC_East_Units + [_entry]}};
					case "GUER":	{
										if(DAC_Res_Side == 0) then
										{
											if(!(_entry in DAC_East_Units)) then {DAC_East_Units = DAC_East_Units + [_entry]};
										}
										else
										{
											if(!(_entry in DAC_West_Units)) then {DAC_West_Units = DAC_West_Units + [_entry]};
										};
									};
					default			{};
				};
