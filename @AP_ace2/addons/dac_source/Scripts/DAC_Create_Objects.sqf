...
if(isServer) then
{		
	_scr = _this execVM "\DAC_Source\Scripts\DAC_Check_ObjScr.sqf";
	...
	if(DAC_Basic_Value != 10) then
	{
		...
		if((_wpc + _DAC_WP_Typ) == 0) then
		{
			...
			[(_this select 0),_DAC_Count_WP,_obj_Nr,_inst,_objArray,(_this select 7)] execVM "\DAC_Source\Scripts\DAC_SimplePos_Objects.sqf";
		}
		else
		{
			if(count _objArray > 1) then
			...
			if(count _this == 8) then
			{
				if(_inst > 0) then
				{
					...
					{
						[(_this select 0),(_DAC_Count_WP - 0.5),_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "\DAC_Source\Scripts\DAC_Create_Objects.sqf";
					}
					else
					{
						[(_this select 0),_DAC_Count_WP,_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "\DAC_Source\Scripts\DAC_Create_Objects.sqf";
					};
				...
				switch (_run) do
				{
					case 3:	{
								...
								else
								{
										...
										if(format["%1",_newObjArray] != "ANY") then
										{
											if(DAC_Marker > 0) then
											{
												if((DAC_Marker_Val select 1) > 0) then
												{
													...
													if((DAC_Marker_Val select 1) == 1) then {_MTemp = _MTemp + [_mb]};
												};
											};
											_action = false
										};
									...
									{
										[_KiZone,10,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,0] execVM "\DAC_Source\Marker\DAC_ZoneMarker.sqf";
									}
									else
									{
										[_KiZone,10,0,_DAC_PolyCount,0] execVM "\DAC_Source\Marker\DAC_ZoneMarker.sqf";
					...
					case 11:{
								...
								if(_run != 7) then
								{
									...
									if(format["%1",(_newo select 0)] != "LOGIC") then
									{
										...
										if(!(format["%1",_arrayName] == "")) then {call compile format["%1 = %1 + [_obj]",_arrayName]};
										_TempArray = _TempArray + [_obj];
										...
										{
											if(!((_newo select 0) in _debA)) then {_debA = _debA + [(_newo select 0)]};
										}
										else
										{
									...
									else
									{
										...
										if(!(format["%1",_arrayName] == "")) then
										{
											...
											call compile format["%1 = %1 + [[_RandomX,_RandomY]]",_arrayName];
											_TempArray = _TempArray + [_obj];
											_wpnr = _wpnr + 1;_chnr = 0;
											if(DAC_Marker > 0) then
											{
