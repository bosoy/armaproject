...
// DAC Arrays & Vars;
	...
	DAC_Master_Init = 0;DAC_WPCount_Init = 0;DAC_AI_Path = format["\DAC_Source\AI_%1\",DAC_AI_AddOn];DAC_NewZone = 0;
...
else
{
	While {_pa < count DAC_Players} do 
	{
		...
		{
			{if(!(_x in DAC_Players)) then {DAC_Players = DAC_Players + [_x]}} foreach units _pn;
		};
		_pa = _pa + 1;
	};
};
...
{
	sleep 2;
	[1]execVM "\DAC_Source\Scripts\DAC_Init_Waypoint.sqf";
};
...
DAC_Players = DAC_Players + [DAC_StartDummy];
[]execVM "\DAC_Source\Scripts\DAC_Clean_Arrays.sqf";