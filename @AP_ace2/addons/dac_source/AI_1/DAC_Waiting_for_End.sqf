		...
		switch (_run) do
		{
			...
			case 4:	{
						...
						else
						{
							...
							{_allcampobj = _allcampobj + [_x]} foreach (_camparray select 4);
							{_allcampobj = _allcampobj + [_x]} foreach (_camparray select 6);
							if((count (_camparray select 8)) > 0) then {{_allcampobj = _allcampobj + [_x]} foreach ((_camparray select 8) select 0)};
							...
							else
							{
								...
								if(!(format["%1",_tempbuild] == "<NULL-object>")) then
								{
									if((format["%1",call compile _tempvalue] == format["%1",call compile _tempvalue2]) && (_tempcount > 0)) then
									{
										if(_temptyp == 0) then
										{
											_temparray1 = _temparray1 + [_i];
											_temparray2 = _temparray2 + [_i];
										}
										else
										{
											if(_temptyp == 1) then
			...
			case 10:{
						...
						call compile format["_grouppara execVM ""\DAC_Source\Scripts\DAC_Group_%1.sqf""",_scriptTyp select (_groupid - 1)];
