...
{if(_x != (vehicle _x)) then {if(!((vehicle _x) in _vehG)) then {_vehG = _vehG + [(vehicle _x)]}}}foreach units _group;
sleep 0.1;
if(count _vehG > 0) then
{
	while{_i < count _vehG} do
	{
		...
		{
			{if((_x != (driver (_vehG select _i))) && (_x != (gunner (_vehG select _i))) && (_x != (commander (_vehG select _i)))) then {_allCargo = _allCargo + [[_x,(_vehG select _i)]]}}foreach _cargoA;
		};
		_i = _i + 1;
	};
};
