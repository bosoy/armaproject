...
if(({alive _x} count units _grouponfire) > 0) then
{
	...
	if(_action) then
	{
		...
		if(!(_grouponfire in DAC_Call_Groups)) then {DAC_Call_Groups = DAC_Call_Groups + [_grouponfire]};
		...
		while{_run} do
		{
			while{_go} do
			{
				...
				{
					_tempgroups = _tempgroups + [_checkgroup];
					if((count _tempgroups) == (count _TempAll)) then
					{
						_tempgroups = _tempgroups + [_grouponfire];
						_run = false;
						_go = false
					};
				};
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									[_hgroup] spawn DAC_fWaitForHelp;_DAC_Temp_Groups = _DAC_Temp_Groups + [_hgroup];
