...
{
	_uArray = _uArray + [format["%1",side _leader]];
	switch (format["%1",side _leader]) do
	{
		case "WEST":	{if(DAC_Res_Side == 1) then {_uArray = _uArray + ["GUER"]}};
		case "EAST":	{if(DAC_Res_Side == 0) then {_uArray = _uArray + ["GUER"]}};
		case "GUER":	{
							if(DAC_Res_Side == 1) then {_uArray = _uArray + ["WEST"]};
							if(DAC_Res_Side == 0) then {_uArray = _uArray + ["EAST"]};
						};
		default {_i = 3;_a = 0};
	};
	...
	while {_i < 3} do
	{
		switch (_i) do
		{
			case 0:	{
						...
						if(_i < 3) then {{if((_x != vehicle _x) && (!((vehicle _x) in _veh))) then {_veh = _veh + [vehicle _x]}} foreach (units _enemyGroup)};
			...
			case 2:	{
						...
						{
							{
								...
								&& (format["%1",gunner (_x select 0)] != "<NULL-object>")) then
								{
									...
									if(((_x select 0) distance _target) < _maxdistance) then
									{
										...
										{
											_artiVeh = _artiVeh + [[(_x select 0),(_x select 1)]];
											DAC_Arti_Veh = DAC_Arti_Veh + [(_x select 0)];
										}
										else
										{
						...
						if((_i < 3) && (((count _addcondition) > 0) || ((count _artiVeh) > 0))) then
						{
							...
							{
								_tempV = _tempV + [((_addcondition select _n) select 0)];
								_tempA = _tempA + [((_addcondition select _n) select 1)];
								_n = _n + 1;
							};
												...
												if(!(_artiUnit in DAC_Arti_Veh)) then
												{
													if(!(_artiUnit in _tArray)) then
													{
														if((_condition select 6) >= 1) then
														{
															...
															if(_f >= 0) then
															{
																_artiVeh = _artiVeh + [[_artiUnit,(_tempA select _f)]];
																_tArray = _tArray + [_artiUnit];
															};
														}
														else
														{
		...
		{	
			if(count _artiVeh > (_condition select 4)) then
			{
				while {(count _sArray < (_condition select 4))} do
				{
					...
					{
						_cArray = _cArray + [_val];
						_sArray = _sArray + [(_artiVeh select _val)];
						DAC_Arti_Veh = DAC_Arti_Veh + [((_artiVeh select _val) select 0)];
					};
				};
			}
			else
			{
				_sArray = _artiVeh;
				{DAC_Arti_Veh = DAC_Arti_Veh + [(_x select 0)]}foreach _artiVeh;
			};
