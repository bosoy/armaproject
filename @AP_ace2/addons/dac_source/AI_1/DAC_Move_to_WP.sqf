...
while {_run > 0} do
{
	switch (_run) do
	{
		case 1:	{
					...
					else
					{
						...
						else
						{
							...
							{
								if(((_zskill select 11) select 0) == 0) then {if(!(_group in DAC_No_Support)) then {DAC_No_Support = DAC_No_Support + [_group]}};
		...
		case 12:{
					...
					_newveh = (_veh select 0);DAC_VehicleInUse = DAC_VehicleInUse + [_newveh];
					_groupveh = _groupveh + [_newveh];DAC_VehicleForUse = DAC_VehicleForUse - [_newveh];
		...
		case 17:{
					...
					else
					{
						...
						else
						{
							...
							{
								_callg = [];
								{if((!(side _x == side leader _group)) && (((leader _group) knowsabout _x) > 0) && (alive _x))  then {_callg = _callg + [_x]}} foreach DAC_Fire_Pos;
							...
							else
							{
								...
								if(!(_group in DAC_Help_Groups)) then {DAC_Help_Groups = DAC_Help_Groups + [_group]};
		...
		case 18:{
					...
					{if(side (leader _x ) == side leader _group) then {_callg = _callg + [_x]}} foreach DAC_Call_Groups;
					if((count _callg) == 0) then
					{
