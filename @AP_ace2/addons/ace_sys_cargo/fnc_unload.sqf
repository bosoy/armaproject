...
_index = lbCurSel _ctrl; TRACE_1("",_index);
...
if (_cargo in _currentcontent) then {
	...
	_unloadPos = [(_posV select 0) + ((sin _dir) * _distance), (_posV select 1) + ((cos _dir) * _distance),_posV select 2];	//diag_log _unloadPos;
	...
	if (_cargo isKindOf "ACE_Rucksack_crate") then {
		...
		if (_cargo_owner == name player) then {
			...
			[_cargo,player,nil,_backpack] execVM "\x\ace\addons\sys_ruck\act_TakeRuck.sqf";
		};
	};
