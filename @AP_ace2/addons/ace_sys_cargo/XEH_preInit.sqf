...
FUNC(weight) = {
	...
	_baseweight = if (isNumber(configFile >> "CfgVehicles" >> typeOf _cargo >> "ACE_Weight")) then {
		getNumber(configFile >> "CfgVehicles" >> typeOf _cargo >> "ACE_Weight");
	} else {
		(_volume * 0.5);
	};
...
FUNC(dragkeydown) = {
	...
	_dikCode_check = actionKeys "stand" + actionKeys "prone" + actionkeys "crouch" + actionkeys "moveup" + actionkeys "movedown"; //stop standing up while dragging
