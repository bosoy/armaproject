...
switch _action do {
	case "start": {
		...
		if ((_unit distance _box) <= 2.5) then {
			...
			[_unit,_weight] call ace_sys_stamina_fnc_inc_mass; // TODO: Get Weight
			if (_weight > 13) then {
		...
		while { _unit getVariable QGVAR(carrying) } do {
			...
			_animdrop = (animationState _unit) in ["aidlpercmstpsraswrfldnon_idlesteady04","aidlpercmstpsraswrfldnon_aiming01","aidlpercmstpsraswrfldnon_idlesteady02","aidlpercmstpsraswrfldnon_idlesteady03","aidlpercmstpsraswrfldnon_idlesteady01","aidlpercmstpsraswrfldnon_aiming02"];
	...
	case "stop": {
		...
		//_weight = [_box] call FUNC(weight);
		[_unit,0] call ace_sys_stamina_fnc_inc_mass; // TODO: Get Weight
