...
switch (toLower ACE_SYS_SA_RFL_WCFG_ELEVATION_UNITS) do {
	case "mil": { ACE_SYS_SA_RFL_SET_CURELEVATION_DEG_LOCAL(deg ((_curelev - _defaultelev - _offset)/1000)) };
	case "mil_e": { ACE_SYS_SA_RFL_SET_CURELEVATION_DEG_LOCAL((_curelev - _defaultelev - _offset)*0.06) };
	case "moa": { ACE_SYS_SA_RFL_SET_CURELEVATION_DEG_LOCAL((_curelev - _defaultelev - _offset)/60) };
	case "deg": { ACE_SYS_SA_RFL_SET_CURELEVATION_DEG_LOCAL(_curelev - _defaultelev - _offset) };
	default { ACE_SYS_SA_RFL_SET_CURELEVATION_DEG_LOCAL(_curelev - _defaultelev - _offset) };
};

switch (toLower ACE_SYS_SA_RFL_WCFG_WINDAGE_UNITS) do {
	case "mil": { ACE_SYS_SA_RFL_SET_CURWINDAGE_DEG_LOCAL(deg ((_curwind - _defaultwind)/1000)) };
	case "mil_e": { ACE_SYS_SA_RFL_SET_CURWINDAGE_DEG_LOCAL((_curwind - _defaultwind)*0.06) };
	case "moa": { ACE_SYS_SA_RFL_SET_CURWINDAGE_DEG_LOCAL((_curwind - _defaultwind)/60) };
	case "deg": { ACE_SYS_SA_RFL_SET_CURWINDAGE_DEG_LOCAL(_curwind - _defaultwind) };
	default { ACE_SYS_SA_RFL_SET_CURWINDAGE_DEG_LOCAL(_curwind - _defaultwind) };
};