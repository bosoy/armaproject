...
while {true} do {
	if (alive player) then {
		if (vehicle player == player) then {
			...
			if !(_vehicle isKindOf "ParachuteBase" || {_vehicle isKindOf "StaticWeapon"} || {_vehicle isKindOf "Motorcycle"} || {_vehicle isKindOf "BIS_Steerable_Parachute"} || {_vehicle isKindOf "ATV_Base_EP1"}) then {
				_side = getNumber (configFile>>"CfgVehicles" >> typeOf _vehicle >> "side");
				if (_side in [0,1,2]) then {
					if (_side == 0) then {
						_array = GVAR(rus_vec_radio) select 0;
